...
def comparison_results():
    ...
    city1 = getWeather(city1, units)
    city2 = getWeather(city2, units)
    ...
    dt1 = datetime.fromtimestamp(city1["sys"]["sunset"]) # 1973-11-29 22:33:09
    dt2 = datetime.fromtimestamp(city2["sys"]["sunset"]) # 1977-06-07 23:44:50
    ...
    context = {
        ...
        "city1_info": {
            ...
            'temp': int(city1["main"]["temp"]),
            'humidity': city1["main"]["humidity"],
            'wind_speed': city1["wind"]["speed"],
            'sunrise': city1["sys"]["sunrise"],
            'sunset': int(city1["sys"]["sunset"]),
            'units_letter': get_letter_for_units(units)
        },
        "city2_info": {
            'date': datetime.now(),
            ...
            'temp': int(city2["main"]["temp"]),
            'humidity': city2["main"]["humidity"],
            'wind_speed': city2["wind"]["speed"],
            'sunrise': city2["sys"]["sunrise"],
            'sunset': int(city2["sys"]["sunset"]),
            'units_letter': get_letter_for_units(units)
        }
    }
